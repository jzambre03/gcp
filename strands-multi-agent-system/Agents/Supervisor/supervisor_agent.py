...
def execute_worker_pipeline(
    ...
    """
    Execute the 2-agent file-based pipeline with dynamic branch creation and policy-aware analysis.
    
    This orchestrates two worker agents in sequence using file-based communication:
    1. Config Collector - Validates golden branch, creates drift branch, runs drift.py, saves to context_bundle.json
...
def run_validation(
    ...
    instruction = f"""
    Please orchestrate the complete 2-agent file-based validation workflow with dynamic branch management:
    ...
    2. Execute the 2-agent file-based pipeline:
       - Config Collector: Validate golden branch exists, create unique drift branch from main, run drift.py, save to context_bundle.json
       - Diff Engine: Read context_bundle, analyze with AI, save to enhanced_analysis.json
