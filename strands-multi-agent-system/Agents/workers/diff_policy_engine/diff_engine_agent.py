...
- Be thorough in your analysis but concise in descriptions"""
    ...
    def _fallback_llm_categorization(self, deltas: list, file: str) -> dict:
        ...
        - high/medium/low: id, file, locator, old, new, drift_category, why, ai_review_assistant, remediation
        ...
        for delta in deltas:
            ...
            if bucket == 'allowed_variance':
                ...
                # Generate why, potential_risk, and suggested_action based on bucket
                if 'password' in old_val or 'secret' in old_val:
                    ...
                    item["ai_review_assistant"] = {
                        "potential_risk": "Service outage if credentials are invalid. Authentication failures will occur. Users may be locked out.",
                        "suggested_action": "Verify credentials with security team. Test authentication in staging. Update vault/secrets manager. Deploy during maintenance window."
                    }
                elif 'jdbc' in old_val or 'datasource' in old_val:
                    item["why"] = f"Database connection changed from {old_str} to {new_str}"
                    item["ai_review_assistant"] = {
                        "potential_risk": "Database connection failures causing 500 errors. Data operations will fail. Application cannot start.",
                        "suggested_action": "Verify connection string with DBA team. Test connectivity in staging. Ensure database exists and is accessible. Monitor error logs after deployment."
                    }
                elif 'url' in old_val or 'endpoint' in old_val:
                    item["why"] = f"Network endpoint changed from {old_str} to {new_str}"
                    item["ai_review_assistant"] = {
                        "potential_risk": "API calls may fail if endpoint is incorrect. Integration with external services will break. Timeouts or 404 errors expected.",
                        "suggested_action": "Verify endpoint with service owner. Test connectivity. Check firewall rules. Update DNS if needed. Monitor integration health."
                    }
                elif category.startswith('dependency'):
                    item["why"] = f"Dependency modified: {category}"
                    item["ai_review_assistant"] = {
                        "potential_risk": "Breaking API changes in new version. Security vulnerabilities if older version. Incompatibility with other dependencies.",
                        "suggested_action": "Review dependency changelog. Run full test suite. Check for CVEs. Verify compatibility with other libraries. Test in staging first."
                    }
                else:
                    item["why"] = f"Configuration changed from {old_str} to {new_str}"
                    item["ai_review_assistant"] = {
                        "potential_risk": "Unexpected behavior change. May affect performance or functionality. Review needed to assess impact.",
                        "suggested_action": "Review change rationale. Test affected functionality. Verify no side effects. Document reason for change. Monitor metrics after deployment."
                    }
    ...
    def _merge_llm_outputs(self, llm_outputs: list, overview: dict, context_bundle: dict) -> dict:
        ...
        # Count files with drift from MERGED output (not context_bundle)
        # This ensures we only count files that made it into the final categorized output
        all_merged_items = (
            merged.get('high', []) + 
            merged.get('medium', []) + 
            merged.get('low', []) + 
